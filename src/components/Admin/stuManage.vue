...
<template>
  <div class="stuMag">
    ...
    <Card style="margin-top:24px">
      <h2><Icon type="md-cloud-upload" /> 导入学生表至数据库</h2>
      <p>将 Excel 表格的数据导入到系统中</p>
      <Divider />
      <Alert type="warning" show-icon>
        注意！这很重要.
        <template slot="desc">
          请按照下面这张图片的格式来编辑您的学生数据表格（每列顺序可变但字段名要与图片中保持一致）。
        </template>
      </Alert>
      <p style="text-align: center;">
        <img src="../../assets/stuTable.jpeg" width="600px">
      </p>
      <Upload
        :multiple=false
        type="drag"
    ...
    <Card style="margin-top:24px">
      <h2><Icon type="md-trash" /> 清空学生表</h2>
      <p>将学生表中的数据清空</p>
      <Divider />
      <Alert type="warning" show-icon>
        注意！这很重要.
...
export default {
  ...
  methods: {
    ...
    async truncate () {
      const res = await this.$store.dispatch('global/truncate', {jwt: this.jwt, role: 'student'})
      if (res)
        this.$Notice.success({
          title: this.msg,
          duration: 5
